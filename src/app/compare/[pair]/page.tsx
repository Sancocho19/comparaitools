...
function CompRow({ label, valA, valB, highlight }: { label: string; valA: React.ReactNode; valB: React.ReactNode; highlight?: "a" | "b" | null }) {
  return (
    <div className="grid grid-cols-[140px_1fr_1fr] md:grid-cols-[180px_1fr_1fr] text-sm" style={{ borderBottom: "1px solid var(--border)" }}>
      ...
      <div className="p-3 md:p-4 text-center" style={{ color: highlight === "a" ? "var(--accent)" : "var(--text)", fontWeight: highlight === "a" ? 700 : 400 }}>{valA}</div>
      <div className="p-3 md:p-4 text-center" style={{ color: highlight === "b" ? "var(--accent)" : "var(--text)", fontWeight: highlight === "b" ? 700 : 400 }}>{valB}</div>
    </div>
  );
}
...
export default async function ComparePage({ params }: { params: Promise<{ pair: string }> }) {
  ...
  return (
    <>
      ...
      <main className="max-w-[850px] mx-auto px-6 md:px-8 py-12 relative z-10">
        <div className="flex items-center justify-center gap-2 text-xs text-[var(--text-dim)] mb-8">
          <Link href="/" className="hover:text-[var(--accent)]">Home</Link>
          <span>/</span>
          <Link href="/tools" className="hover:text-[var(--accent)]">Compare</Link>
          <span>/</span>
        ...
        <div className="rounded-2xl overflow-hidden mb-8" style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
          ...
          <CompRow label="Rating" valA={`${"★".repeat(Math.floor(toolA.rating))} ${toolA.rating}`} valB={`${"★".repeat(Math.floor(toolB.rating))} ${toolB.rating}`} highlight={toolA.rating > toolB.rating ? "a" : toolB.rating > toolA.rating ? "b" : null} />
          ...
          <CompRow label="Growth" valA={toolA.trend} valB={toolB.trend} highlight={parseInt(toolA.trend) > parseInt(toolB.trend) ? "a" : parseInt(toolB.trend) > parseInt(toolA.trend) ? "b" : null} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          {[toolA, toolB].map((tool) => (
            <div key={tool.id} className="rounded-2xl p-6 md:p-7" style={{ background: "var(--bg-card)", border: `1px solid ${tool === winner ? "var(--accent)" : "var(--border)"}` }}>
              ...
              <div className="pt-4" style={{ borderTop: "1px solid var(--border)" }}>
                ...
                {tool.pros.slice(0, 2).map((p, i) => (
                  <p key={i} className="text-[var(--text-muted)] text-xs mb-1.5">✓ {p}</p>
                ))}
                <h4 className="text-xs font-bold text-[var(--red)] mt-3 mb-2">Cons</h4>
                {tool.cons.slice(0, 2).map((c, i) => (
                  <p key={i} className="text-[var(--text-muted)] text-xs mb-1.5">✗ {c}</p>
                ))}
              </div>
            </div>
          ))}
        ...
        <div className="flex gap-4 justify-center flex-wrap">
          {[toolA, toolB].map((tool) => (
            <Link key={tool.id} href={`/tools/${tool.slug}`} className="px-6 py-3 rounded-xl text-sm font-semibold transition-all hover:scale-105" style={{ background: "var(--bg-card)", border: "1px solid var(--border)", color: "var(--text)" }}>
