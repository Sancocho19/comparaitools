...
const TYPE_LABELS: Record<string, { label: string; color: string }> = {
  ...
  trending:   { label: 'Trending',    color: '#f43f5e' },
};
...
export default async function BlogPage() {
  ...
  const trending    = sorted.filter(p => p.category === 'trending');
      ...
      <main className="max-w-[1200px] mx-auto px-6 py-12 relative z-10">
        ...
        <div className="grid grid-cols-5 gap-4 mb-12">
          {[
            ...
            { label: 'Trending',     count: trending.length,    color: '#f43f5e' },
        ...
        {sorted.length === 0 ? (
          ...
          <>
            {/* Trending — investigación real con web search */}
            {trending.length > 0 && (
              <>
                <div className="flex items-center gap-3 mb-5">
                  <h2 className="text-xl font-bold text-[var(--text)]">🔥 Trending Now</h2>
                  <span className="text-[11px] px-2.5 py-1 rounded-full font-bold animate-pulse"
                ...
                <p className="text-[13px] mb-6" style={{ color: 'var(--text-dim)' }}>
                  Articles written after researching real AI trends happening right now — not templated content.
                </p>
            ...
            <div className="blog-grid mb-12">
              {sorted.filter(p => p.category !== 'trending').map(post => (
                <PostCard key={post.slug} post={post} />
              ))}
            </div>
          </>
