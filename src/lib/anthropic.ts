...
export async function generateComparison(toolA: any, toolB: any) {
  const message = await anthropic.messages.create({
    ...
    messages: [{
      role: "user",
      content: `Write an SEO-optimized comparison: "${toolA.name} vs ${toolB.name} (2026)". Tool A: ${JSON.stringify(toolA)}. Tool B: ${JSON.stringify(toolB)}. Include: compelling intro, feature-by-feature comparison table, pricing comparison, use cases where each wins, final verdict, FAQ section. Target keywords: "${toolA.name} vs ${toolB.name}". Format as semantic HTML.`
    }]
  });
  return message.content[0].type === 'text' ? message.content[0].text : '';
}
...
## Archivo 2: API Route
```
mkdir src\app\api\auto-generate
notepad src\app\api\auto-generate\route.ts